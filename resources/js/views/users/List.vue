<template>
  <div class="app-container">
    ...
    <el-dialog :title="'Edit User'" :visible.sync="dialogFormEditVisible">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      nonAdminRoles: ['manager', 'editor', 'user', 'visitor'],
      newUser: {},
  ...
  methods: {
    ...
    handleEdit(id) {
      ...
      this.userEdit = {
        id: found.id,
        name: found.name,
        role: found.roles.join(', '),
        email: found.email,
      };
      // this.resetEdit();
      this.$nextTick(() => {
        this.$refs['userFormEdit'].clearValidate();
      });
    },
    ...
    editUser() {
      this.$refs['userFormEdit'].validate((valid) => {
        if (valid) {
          this.userEdit.roles = [this.userEdit.role];
